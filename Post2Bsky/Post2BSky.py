...
class BlueSkyBot:
    def __init__(self, config=None, config_file=None):
        """
        コンストラクタ：設定ファイルまたは環境変数から設定を読み込み、各APIの初期化を行います。
        """
        if config_file:
            ...
            self.config = {
                ...
                'gspread_json_file': os.getenv('GSPREAD_CREDENTIALS_FILE', '/secrets/credentials.json'),
                'spreadsheet_key': os.getenv('SPREADSHEET_KEY')
            }

        missing = [k for k, v in self.config.items() if not v and k not in ('gspread_json_file',)]
        if missing:
